...
const char * const PIVX_CONF_FILENAME = "wallacoin.conf";
const char * const PIVX_PID_FILENAME = "wallacoin.pid";
const char * const PIVX_MASTERNODE_CONF_FILENAME = "masternode.conf";


// Wallacoin only features
// Masternode
...
#else
    const char* pszModule = "wallacoin";
#endif
    if (pex)
        return strprintf(
...
{
// Windows < Vista: C:\Documents and Settings\Username\Application Data\wallacoin
// Windows >= Vista: C:\Users\Username\AppData\Roaming\wallacoin
// Mac: ~/Library/Application Support/wallacoin
// Unix: ~/.wallacoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Wallacoin";
#else
    fs::path pathRet;
...
static fs::path ZC_GetBaseParamsDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\wallacoinParams
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\wallacoinParams
    // Mac: ~/Library/Application Support/wallacoinParams
    // Unix: ~/.wallacoin-params
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "WallacoinParams";
#else
    fs::path pathRet;
    char* pszHome = getenv("HOME");
    if (pszHome == NULL || strlen(pszHome) == 0)
    ...
    pathRet /= "Library/Application Support";
    TryCreateDirectory(pathRet);
    return pathRet / "WallacoinParams";
#else
    // Unix
    return pathRet / ".wallacoin-params";
#endif
#endif
}
    ...
    std::map<std::string, std::vector<std::string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty wallacoin.conf if it does not exist
        FILE* configFile = fsbridge::fopen(GetConfigFile(), "a");
        if (configFile != NULL){
            std::string strHeader = "# Wallacoin config file\n"
                          "rpcuser=username\n"
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override wallacoin.conf
