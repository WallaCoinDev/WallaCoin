    ...
    ui(new Ui::DashboardWidget)
{
    ...
    // Staking Information
    ui->labelMessage->setText(tr("Amount of WALLA earned via Staking & Masternodes"));
	...
	ui->labelAmountMNRewards->setText("0 WALLA");
...
void DashboardWidget::loadWalletModel()
{
 ...
 ///       connect(ui->btnHowTo, &QPushButton::clicked, window);
    ...
    // update the display unit, to not use the default ("WALLA")
    updateDisplayUnit();
}
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    chartData->amountsByCache = getAmountBy(); // pair WALLA, zWALLA
...
void DashboardWidget::onChartRefreshed()
{
    ...
    set0->setColor(QColor(21,210,144));
	set1->setColor(QColor(22,176,179));
