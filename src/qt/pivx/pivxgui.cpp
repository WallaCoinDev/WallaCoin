...
void PIVXGUI::createTrayIcon(const NetworkStyle* networkStyle)
{
    ...
    QString toolTip = tr("Wallacoin client") + " " + networkStyle->getTitleAddText();
...
{
    QString strTitle =  tr("Wallacoin"); // default title
...
bool PIVXGUI::openStandardDialog(QString title, QString body, QString okBtn, QString cancelBtn)
{
    ...
    if (isVisible()) {
        ...
        dialog->setWindowTitle(tr("Wallacoin"));
        dialog->adjustSize();
        dialog->raise();
        dialog->exec();
    }
