...
class ReorgStakeTest(PivxTestFramework):
    ...
    def check_money_supply(self, expected_piv, expected_zpiv):
        ...
        # verify that nodes have the expected WALLA and zWALLA supply
        for node in g_info:
            assert_equal(node['moneysupply'], DecimalAmt(expected_piv))
            for denom in node['zWALLAsupply']:
                assert_equal(node['zWALLAsupply'][denom], DecimalAmt(expected_zpiv[denom]))
    ...
    def run_test(self):
        ...
        # Check WALLA and zWALLA supply at the beginning
        # ------------------------------------------
        # zWALLA supply: 2 coins for each denomination
        ...
        # WALLA supply: block rewards minus burned fees for minting
        ...
        # Verify that WALLA and zWALLA supplies were properly updated after the spends and reorgs
        self.log.info("Check WALLA and zWALLA supply...")
